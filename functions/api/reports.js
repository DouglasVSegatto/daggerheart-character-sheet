export async function onRequestGet(context) {
  ...
  const list = await env.character_sheet_reports.list({ limit: 100 });
  const reports = await Promise.all(list.keys.map(k => env.character_sheet_reports.get(k.name, { type: 'json' })));
